...
async function criarTarefa(clientes) {
try {
// Conecta ao banco de dados
const connection = await connectDatabase();
// Cria uma nova solicitação para inserir a nova tarefa na tabela 'Tarefas'
const request = new Request(
"INSERT INTO clientes (nome,idade,sexo,cidade) VALUES (@nome,@idade,@sexo,@cidade);",
(err, rowCount) => {
if (err) {
...
request.addParameter("nome",TYPES.VarChar, clientes.nome,);
request.addParameter("idade",TYPES.Int, clientes.idade,);
request.addParameter("sexo",TYPES.Char, clientes.sexo,);
request.addParameter("cidade",TYPES.VarChar, clientes.cidade,);
// Executa a solicitação no banco de dados
connection.execSql(request);
} catch (error) {
// Se ocorrer um erro, registra o erro no console e lança uma exceção
...
const request = new Request(
`UPDATE clientes SET nome = @nome, idade = @idade, sexo = @sexo, cidade = @cidade WHERE cliente_id = @id;`,
(err, rowCount) => {
if (err) {
// Se houver erro na execução da solicitação
reject(err); // Rejeita a promessa com o erro
...
resolve({ id,nome,idade,sexo,cidade }); // Resolve a promessa com os dados da tarefa atuali
} else {
// Se nenhum registro foi atualizado (tarefa não encontrada)
reject(new Error("Cliente não encontrada.")); // Rejeita a promessa com uma nova
}
}
}
);
...
request.addParameter("sexo", TYPES.Char, sexo);
